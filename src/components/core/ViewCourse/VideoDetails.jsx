...
const VideoDetails = () => {
  ...
  const nextLecture = () => {
    ...
    const currentSectionIndex = courseSectionData?.findIndex((section) => section._id === sectionId);
    console.log("currsec",currentSectionIndex);
    const currentSubsectionIndex = courseSectionData[currentSectionIndex]?.SubSection.findIndex((subsection) => subsection._id === subsectionId);
    console.log("currSubsec",currentSubsectionIndex);
    if (currentSubsectionIndex === courseSectionData[currentSectionIndex]?.SubSection.length - 1) {
      ...
      console.log("nextsubsection",nextSubsectionId);
    }else {
      const nextSectionId = courseSectionData[currentSectionIndex]._id;
            console.log("nextsectionid1",nextSectionId);

      const nextSubsectionId = courseSectionData[currentSectionIndex].SubSection[currentSubsectionIndex + 1]._id;
            console.log("nextsubsection1",nextSubsectionId);
                  ...
                  <div className='flex justify-center items-center'>
                    {
                      !completedLectures.includes(videoData._id) && (
                        <button onClick={()=>{handleLectureCompletion()}} className='bg-yellow-200 text-richblack-900 absolute top-[20%] hover:scale-90 z-20 font-medium md:text-sm px-4 py-2 rounded-md'>Mark as Completed</button>
                      )
                    }
                  </div>
                  {
