...
const Upload = ({name, label, register, errors, setValue}) => {
    ...
    const handelonchange = (e) => {
        ...
        if(file) {
            ...
            // FileReader: This is a JavaScript API 
            // used to read files stored on the user's computer asynchronously.
            reader.onloadend = () => {
              // onloadend: This event triggers when the 
              // reading is finished.
                setimage(reader.result);

                // setimage(reader.result): Takes that base64 string and 
                // puts it into the local image state. T
            }
            reader.readAsDataURL(file);
            // readAsDataURL(file): Starts reading the file. It converts the image into a "base64 string" 
            // (a very long string of text that represents the image).
        }
        else {
            console.log("no file");
        }
