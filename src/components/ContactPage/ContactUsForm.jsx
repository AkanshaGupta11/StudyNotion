...
//useform --.react hook form --> manage entire form state , validation and submission 
// without this --. u would have to write for each  onchange 
function ContactUsForm() {
    const[loading , setLoading] =  useState(false);
    const{
        register, // connect input to manager
        handleSubmit, // handle validation / submission 
        reset, 
    ...
    useEffect(() => {
        //form successfully submit hogya -> tho fields ko rest kr do 
		...
		loading ? (<div className=".custom-loader w-[100%] pt-[30%] pb-[30%]"><div className="custom-loader"></div></div>) : (
			<div>
				<form onSubmit={handleSubmit(onSubmit)} className={"flex flex-col gap-7"}>
					...
					<div className='flex flex-col gap-2'>
						...
						<div className='flex gap-5'>
							<div className='flex w-[81px] flex-col gap-2'>
								...
								name="countrycode" 
								id="countryCode" 
								className="form-style" {...register("countryCode", { required: true })}>
									{
					...
					<button type="submit" className="rounded-md bg-yellow-200 px-6 py-3 text-center text-[13px] font-bold text-black shadow-[2px_2px_0px_0px_rgba(255,255,255,0.18)] transition-all duration-200 hover:scale-95 hover:shadow-none  disabled:bg-richblack-500 sm:text-[16px] ">Send Message</button>
